...
CatCommandProcessorAsyncE.prototype.processCommand = function(command, params, callBack){
	...
	var readSequence=0;
	//Create a closure to hold the calling sequence
	var  callBackFactory = function (sequence){
		...
		return function(err, bytesRead, buffer){
				...
				if(bytesRead==1000){
					...
					fs.read(fileDescriptor, buffer, 0, 1000, null, callBackFactory(readSequence++));
				}else{
		...
		if(exists){
			...
			fs.stat(path, function(err, stat){
				...
				fs.open(path, "r", function(err, fd){
					...
					fs.read(fd, buffer, 0, 1000, null, callBackFactory(readSequence++));
				});
